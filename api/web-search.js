...
​
		const maxResults = Math.min(Math.max(parseInt(body.max_results, 10) || 8, 1), 15);
		const includeDomains = Array.isArray(body.include_domains) ? body.include_domains.filter(Boolean).slice(0, 25) : [];
​
		const tavilyPayload = {
